...
var _ = Describe("In-memory link manager tests", func() {
	...
	It("should delete a link", func() {
		...
		// Delete the link
		err = linkManager.DeleteLink("Brijesh Lakkad", r.Url)
		Ω(err).Should(BeNil())
	...
	It("should update link status when receiving OnLinkChecked() calls", func() {
		...
		linkCheckSink.OnLinkChecked("Brijesh Lakkad", r.Url, om.LinkStatusValid)
		...
		linkCheckSink.OnLinkChecked("Brijesh Lakkad", r.Url, om.LinkStatusInvalid)
